...
```java
rewardAd.setPlacementName("reward_main");
```
...
#### 5. 보상형 SSV(서버 사이드 검증) <a href="#id-2.-initialize" id="id-2.-initialize"></a>
...
map.put("custom_test_key", "custom_test_value");
map.put("os", "Android");
adWhaleMediationRewardAd.setCustomData(map); // custom data (json 직렬화 문자열 처리)
...
* AdMob 공식 가이드: [https://developers.google.com/admob/android/ssv](https://developers.google.com/admob/android/ssv)
...
```java
// 내부 처리 (참고)
new JSONObject(customData).toString();
```
...
val map = mutableMapOf(
    "custom_test_key" to "custom_test_value",
    "os" to "Android"
)
adWhaleMediationRewardAd.setCustomData(map) // custom data (json 직렬화 문자열 처리)
adWhaleMediationRewardAd.setUserId("test_user_id") // 앱 사용자 id
...
```java
// 내부 처리 (참고)
JSONObject(customData).toString()
```
...
#### 6. 보상형 광고 샘플코드 <a href="#id-2.-initialize" id="id-2.-initialize"></a>

...
{% tab title="Swift" %}
```swift
AdWhaleAds.sharedInstance.setTagForChildDirectedTreatment(true)
```
{% endtab %}

{% tab title="Objective-C" %}
```objective-c
[[AdWhaleAds sharedInstance] setTagForChildDirectedTreatment:@YES];
```
{% endtab %}
{% endtabs %}

| setTagForChildDirectedTreatment 설정 | 설명                                      |
| ---------------------------------- | --------------------------------------- |
...
* [아동 대상 설정](3.2.md#id-1)과 동시에 true 로 설정하면 안 되며, 이 경우 아동 대상 설정이 우선 적용됩니다.

{% tabs %}
{% tab title="Swift" %}
```swift
AdWhaleAds.sharedInstance.setTagForUnderAgeOfConsent(true)
```
{% endtab %}

{% tab title="Objective-C" %}
```objective-c
[[AdWhaleAds sharedInstance] setTagForUnderAgeOfConsent:@YES];
```
{% endtab %}
{% endtabs %}

<table data-header-hidden><thead><tr><th width="362">setTagForUnderAgeOfConsent 설정</th><th>설명</th></tr></thead><tbody><tr><td>setTagForUnderAgeOfConsent 설정</td><td>설명</td></tr><tr><td>true</td><td>광고 요청이 EEA에 거주하는 동의 연령 미만의 사용자를 대상으로 처리하도록 지정하는 경우</td></tr><tr><td>미설정</td><td>광고 요청이 EEA에 거주하는 동의 연령 미만의 사용자 취급 방법을 지정하지 않는 경우</td></tr></tbody></table>

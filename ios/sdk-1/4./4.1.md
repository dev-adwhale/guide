...
final class SplashViewController: UIViewController, AdWhaleMediationAppOpenAdDelegate {
    ...
    func appOpenAdDidLoad(_ ad: AdWhaleMediationAppOpenAd) {
        // 로드 성공
        // 4. 노출 (로드 완료 후)
        if ad.isAdAvailable {
            ad.show(from: self)
...
// 로드 성공
- (void)appOpenAdDidLoad:(AdWhaleMediationAppOpenAd *)ad {
    // 4. 노출 (로드 완료 후)
    if (ad.isAdAvailable) {
        [ad showFrom:self];
    }
...
```objectivec
// 필수
- (void)appOpenAd:(AdWhaleMediationAppOpenAd *)ad;   // 광고 요청 성공 시
```
...
* 광고는 로드가 완료된 후에만 노출할 수 있습니다. \
  `appOpenAd(_:)` 이후에만 `show(from:)` 을 호출하세요.

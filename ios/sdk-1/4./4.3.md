...
{% tab title="Swift" %}
```swift
func interstitial(_ ad: AdWhaleMediationInterstitialAd)   // 광고 요청 성공 시
```
...
```objectivec
// 필수
- (void)interstitial:(AdWhaleMediationInterstitialAd *)ad;   // 광고 요청 성공 시
```
...
* 광고는 로드가 완료된 후에만 노출할 수 있습니다. \
  `interstitial(_:)` 이후에만 `show(from:)` 을 호출하세요.

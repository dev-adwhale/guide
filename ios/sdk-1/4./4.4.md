...
// 로드 성공
- (void)rewardedAdDidLoad:(AdWhaleMediationRewardAd *)ad { }
...
func rewardedAd(_ ad: AdWhaleMediationRewardAd)   // 광고 요청 성공 시
```
...
```objectivec
// AdWhaleMediationRewardAdLoadDelegate — 필수
- (void)rewardedAd:(AdWhaleMediationRewardAd *)ad;   // 광고 요청 성공 시
```
...
* 광고는 로드가 완료된 후에만 노출할 수 있습니다. \
  `rewardedAd(_:)` 이후에만 `show(from:rewardDelegate:)` 를 호출하세요.

...
final class NativeViewController: UIViewController, AdWhaleMediationNativeAdDelegate {
    ...
    func nativeAdDidLoad(_ adView: AdWhaleMediationNativeAdView) {
        // 7. 노출 (로드 완료 후)
        adView.show()
    }
...
// 로드 성공
- (void)nativeAdDidLoad:(AdWhaleMediationNativeAdView *)adView {
    // 7. 노출 (로드 완료 후)
    [adView show];
}
...
{% tab title="Swift" %}
```swift
func nativeAd(_ adView: AdWhaleMediationNativeAdView)   // 광고 요청 성공 시
```
...
```objectivec
// 필수
- (void)nativeAd:(AdWhaleMediationNativeAdView *)adView;   // 광고 요청 성공 시
```
...
* 네이티브 광고는 로드 완료 후 `show()` 를 호출해야 노출됩니다. `nativeAd(_:)` 에서 호출하세요.

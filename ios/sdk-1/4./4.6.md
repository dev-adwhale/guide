...
| 델리게이트 메서드                                    | 호출 시점            |  필수 |
| -------------------------------------------- | ---------------- | :-: |
| `popupAd(_:didLoadWith:)`                    | 광고 로드 성공 (잠정)    |  ●  |
...
```swift
func popupAd(_ ad: AdWhaleMediationPopupAd,
             didLoadWith responseInfo: AdWhaleMediationResponseInfo)   // 광고 요청 성공 시 (잠정)
```
...
{% tab title="Objective-C" %}
```objectivec
#import "TransitionPopupViewController.h"
@import AdWhaleSDK;
...
* 필수 콜백은 `popupAd(_:didLoadWith:)` 와 `popupAd(_:didFailToLoadWithError:message:)` 두 개입니다. 나머지 네 개는 optional 이지만, **`popupAd(_:didFailToShowWithError:message:)` 를 구현하지 않으면 재고 부재(`300`)를 알 수 없어 종료 · 전환 플로우가 막힙니다.**

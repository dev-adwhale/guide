...
{% hint style="info" %}
처음 연동하신다면 이 항목은 넘어가고 [1. 요구사항 및 사용권한](1..md) 부터 진행합니다.
{% endhint %}

기존에 사용하시던 **애드웨일 iOS SDK 1.0.x**와 이 문서의 **ADwhale Mediation SDK** 는 모듈 이름은 동일하지만 **다른 제품**입니다. \
클래스 이름과 동작이 전부 달라졌고, 광고를 붙이는 방식도 바뀌었습니다. **이름만 치환해서는 동작하지 않으므로** 앱 코드를 수정해야 합니다.

{% hint style="info" %}
가장 큰 변화는 **지면을 지정하는 방식**입니다.&#x20;

기존에는 앱이 애드몹 광고단위 ID(`adUnitId`)를 직접 넣었지만,\
이제는 **애드웨일 전용 지면 ID(`placementUid`)**&#xB9CC; 넣고 **어떤 네트워크를 어떤 순서로 호출할지는 애드웨일 서버가 내려줍니다.**
{% endhint %}

{% hint style="warning" %}
**두 SDK 를 함께 넣을 수는 없습니다.**&#x20;

두 SDK 의 코어 모듈 이름이 `AdWhaleSDK` 로 같고, 클래스 이름은 다르지만 내부에 포함된 파트너 SDK 가 중복되어 링커 오류나 런타임 오작동이 발생합니다.\
기존 SDK 의존성을 먼저 제거한 뒤 새 SDK 를 추가하세요.
{% endhint %}
...
기존에는 SDK 하나에 지원하는 모든 네트워크가 포함되어 있었습니다. \
이제는 **핵심 모듈에 사용할 네트워크의 어댑터만 골라** 함께 추가합니다.

앱 용량이 줄고, 쓰지 않는 네트워크 때문에 앱 심사에서 챙겨야 할 항목(SKAdNetwork ID 목록, 개인정보 처리 방침 등)도 줄어듭니다.

{% hint style="warning" %}
담은 네트워크는 코드에서 한 줄씩 **등록**해 주어야 합니다.\
등록하지 않으면 그 네트워크는 오류도 크래시도 없이 **조용히 건너뛰어집니다.**
{% endhint %}

#### 실패 원인을 구분해서 알려줍니다.

기존에는 광고를 못 받으면 오류 객체 하나만 왔습니다.\
그래서 **연동을 잘못한 것인지, 그냥 광고가 없는 것인지** 구분하기 어려웠습니다.&#x20;

이제 실패 콜백이 **원인을 구분할 수 있는 코드(`statusCode`)** 를 함께 전달합니다.

<table><thead><tr><th width="127.109375">코드</th><th width="145.265625">의미</th><th>대응</th></tr></thead><tbody><tr><td>200</td><td>연동 오류</td><td>파라미터를 잘못 설정했거나, 광고가 준비되지 않은 상태에서 노출을 요청한 경우입니다. <br>재시도해도 같은 실패가 반복되며, 코드나 설정을 고쳐야 합니다.</td></tr><tr><td>300</td><td>광고 미충족</td><td>설정 응답이 무효하거나, 모든 광고 네트워크에서 광고를 받지 못한 경우입니다.<br>자체 광고로 대체하는 등 후처리가 필요합니다.</td></tr></tbody></table>

{% hint style="info" %}
전체 코드 값은 Error code 를 참고하세요.
{% endhint %}
...
<table><thead><tr><th width="328.3671875">상황</th><th>권장 내용</th></tr></thead><tbody><tr><td>신규 프로젝트</td><td>이 SDK를 사용하세요.</td></tr><tr><td>운영 중인 앱</td><td>다음 정기 업데이트에 맞춰 한 번에 전환하세요.</td></tr><tr><td>Flutter / React Native 플러그인 사용 중인 앱</td><td>아직 전환하지 마세요.<br>각 플러그인이 기존 SDK(1.0.x)를 사용합니다.</td></tr><tr><td>새 광고 포맷(팝업)이나 네트워크가 필요한 앱</td><td>전환이 필요합니다.<br>신규 포맷과 네트워크는 이 SDK 에만 추가됩니다.</td></tr></tbody></table>

{% hint style="info" %}
기존 `1.0.x` 는 계속 동작합니다. 서둘러 전환하실 필요는 없습니다.

다만 **신규 광고 포맷(팝업)** 과 **신규 네트워크**(fill rate를 높이기 위해 LevelPlay · Cauly · AdFit · Admize 등 직접 추가 연동)는 이 SDK 에만 제공됩니다.
{% endhint %}
...
<table><thead><tr><th width="167.73046875">항목</th><th width="247.7421875">기존(1.0.x)</th><th>현재</th></tr></thead><tbody><tr><td>설치</td><td><ul><li>CocoaPods(pod 'AdWhaleSDK')</li><li>Swift Package Manager</li></ul></td><td><ul><li>Swift Package Manager</li></ul></td></tr><tr><td>패키지 구성</td><td>단일 pod(파트너 SDK 내장됨)</td><td>애드웨일 Core + 네트워크별 파트너 어댑터(선택)</td></tr><tr><td>지면 지정</td><td>애드몹 광고단위 ID(<code>adUnitId</code>)</td><td>애드웨일 전용 지면 ID(<code>placementUid</code>)</td></tr><tr><td>워터폴</td><td>애드몹 미디에이션에 위임</td><td>애드웨일 전용 서버가 제어</td></tr><tr><td>진입 클래스</td><td><code>AdWhaleAds.sharedInstance</code> (싱글턴)</td><td><code>AdWhaleMediationAds</code> (static 메서드)</td></tr><tr><td>초기화</td><td><code>intialize(appId:rootViewController:...)</code></td><td><code>initialize(publisherUid:completion:)</code></td></tr><tr><td>실패 콜백</td><td><code>Error</code> 객체</td><td><code>(statusCode: Int, message: String)</code></td></tr><tr><td>rootViewController</td><td>초기화와 광고 객체 모두에 전달 필요</td><td>초기화에서는 불필요(SDK 내부 처리)<br>전체화면 노출 시 <code>show(from:)</code> 으로 전달</td></tr><tr><td>어댑터 등록</td><td>X</td><td>코드상에 <code>register()</code> 호출 필요</td></tr><tr><td>타겟팅 API</td><td><code>setTagForChildDirectedTreatment(Bool)</code><br><code>setTagForUnderAgeOfConsent(Bool)</code><br><code>maxAdContentRating(_:)</code></td><td><code>setCoppa(Bool)</code><br><code>setTagForUnderAgeOfConsent(AdWhaleTagForUnderAgeOfConsent)</code><br><code>setMaxAdContentRating(_:)</code></td></tr></tbody></table>

#### **클래스 변경표**

<table><thead><tr><th width="109.1015625">포맷</th><th width="306.23828125">기존</th><th>현재</th></tr></thead><tbody><tr><td>전역 설정</td><td><code>AdWhaleAds</code></td><td><code>AdWhaleMediationAds</code></td></tr><tr><td>배너</td><td><code>AdWhaleBannerAd</code></td><td><code>AdWhaleMediationAdView</code></td></tr><tr><td>전면</td><td><code>AdWhaleInterstitialAd</code></td><td><code>AdWhaleMediationInterstitialAd</code></td></tr><tr><td>보상형</td><td><code>AdWhaleRewardAd</code></td><td><code>AdWhaleMediationRewardAd</code></td></tr><tr><td>네이티브</td><td><code>AdWhaleNativeAdLoader</code> + <code>AdWhaleNativeAd</code><br>(<code>AdWhaleNativeAdView</code> 상속 · 6개 메서드 오버라이드 필요)</td><td><code>AdWhaleMediationNativeAdView</code> (템플릿)<br>커스텀 바인딩도 지원</td></tr><tr><td>앱오프닝</td><td><code>AdWhaleAppOpenAd.shared</code> (싱글턴)</td><td><code>AdWhaleMediationAppOpenAd</code> (인스턴스 생성)</td></tr><tr><td>팝업</td><td>X</td><td><code>AdWhaleMediationPopupAd</code> (신규)</td></tr></tbody></table>
...
| `Bool`(`setTagForUnderAgeOfConsent`) | `AdWhaleTagForUnderAgeOfConsent` |
| `AdWhaleGdprType`                    | 제거                               |
| `AdWhaleLogLevel`                    | 동일                               |
| X                                    | `AdWhaleNativeTemplate`(신규)      |
| X                                    | `AdWhalePopupCloseReason`(신규)    |
| X                                    | `AdWhaleResultCode`(신규)          |
